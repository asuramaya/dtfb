"""
Server mode: a CarCutter-API-shaped HTTP surface in front of dtfb's own
imaging pipeline, self-hosted.

WHY THIS EXISTS (roadmap item 5, strategy decision 1449fdef, dtfb project
...
speak. dtfb's core pipeline (classify, cutout, composite) does the same
...
dtfb than the CLI (a long-lived process instead of a one-shot script),
which is why it's a separate, OPT-IN dependency group (`pip install
-e ".[server]"`) -- installing dtfb to run it as a script never drags in
...
Run:
    ...
    dtfb-serve --data-dir ~/.dtfb-server --out ~/.dtfb-server/images
"""
...
app = FastAPI(title="dtfb server", description="Self-hosted, CarCutter-API-shaped vehicle imaging service")
...
def shotlist():
    ...
    dtfb's pipeline (there's no separate "shoot" step; images arrive by
...
def main() -> None:
    ...
    parser.add_argument("--data-dir", default="~/.dtfb-server", help="Where the SQLite registry lives")
